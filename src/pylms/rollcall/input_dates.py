...
from ..history import History, get_unmarked_classes
...
def input_class_date(history: History) -> Result[list[str]]:
    ...
    src_dates: list[str] = [date for date in get_unmarked_classes(history, "")]
    if len(src_dates) == 0:
        msg = "no unmarked classes present, all classes marked"
        eprint(msg)
        return Result.err(msg)

    result = select_class_date(msg, dates_in=src_dates)
