from pylms.constants import SPACE_DELIM, SEMI_DELIM


def _clean_irregular_space(entry: str) -> str:
    entry = entry.replace(SPACE_DELIM, "")
    return f"{entry}{SEMI_DELIM}".strip()


def _clean_composite_irregular(entry: str, delim: str) -> str:
    if delim != SPACE_DELIM:
        entries: list[str] = entry.split(delim)
        output: str = ""
        for each_entry in entries:
            output += _clean_irregular_space(each_entry) + SPACE_DELIM
        return output.strip()
    else:
        entry = entry.replace(SPACE_DELIM, "")
        output = ""
