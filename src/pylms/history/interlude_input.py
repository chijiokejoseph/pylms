...
from ..cli import input_dates, input_num, input_option, input_str
...
def new_interlude(history: History) -> Result[Interlude]:
    dates = all_dates(history, "")

    class_date: str

    while True:
        selected = input_dates(
            dates,
            "Select the Class Date when the Interlude starts",
        )
        if selected.is_err():
