...
def parse_to_dates(entry: str) -> list[str]:
    ...
    def match_date_by_index(num_input: int) -> str:
        """
        extracts date that corresponds to the matching number entered as the argument to `num_input`. The returned date is gotten by indexing `dates_list` with `num_input` - 1 since num_input is gotten from a count that starts from 1.

        The program will also forcefully exit with an appropriate message if the num_input - 1 is not a valid index of dates_list or if num_index is less than 0.

        :param num_input: num entered by the user to select a corresponding date from `dates_list`. Since the dates printed out to the user start their count from 1, the index in `dates_list` that matches num_input is always num_input - 1
        :type num_input: int

        :return: date corresponding to the matching number entered as the argument to `num_input`
        :rtype: str
        """
        if len(dates_list) >= num_input > 0:
            ...
            raise InvalidSelectionInputError(
                f"Entered number {choice} is not a valid option from the displayed menu. \nPlease restart the program and try again."
            )
